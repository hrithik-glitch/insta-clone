...
function Modal() {
	...
	const uploadPost = async () => {
		...
		// Get a download URL from firebase storage and update to original post with image
	...
	return (
		<Transition.Root show={open} as={Fragment}>
			<Dialog
				as="div"
				className="overflow-y-auto -inset-48 justify-center text-center fixed sm:z-10 sm:inset-0 sm:overflow-y-auto"
				onClose={setOpen}
			>
					...
					>
						<div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-sm sm:w-full sm:p-6">
							<div>
								{selectedFile ? (
									<img
										className="w-28 h-28 ml-auto mr-auto rounded-md sm:w-full sm:h-[336px]  sm:object-contain sm:cursor-pointer"
										src={selectedFile}
